...
re.client.on("message", (message) => {
    if(config.ows != message.channel.id) return //ignore non-one word story channel
  
    let lastUser = db.get(`${message.channel.id}.lastUser`) // get the ID of the last user to send a word

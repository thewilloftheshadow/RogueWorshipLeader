...
cmds.forEach((x) => {
  let generate = {
    run: async (client, message, args) => {
      ...
      console.log(i)
      let fact = f[i]
      message.channel.send({ embeds: [new re.Discord.MessageEmbed().setTitle("True story...").setThumbnail(message.guild.iconURL()).setDescription(fact.text).setFooter(`ID: ${fact.id}`).setColor(re.vars.randomColor().hexString())] })
    },
    help: {
      name: `${x}`,
